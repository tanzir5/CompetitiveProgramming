...
INF has to be greater or equal to the max capacity in the network
*/
...
int flow[2*MAXE+5], cap[2*MAXE+5], nxt[2*MAXE+5], to[2*MAXE+5];
...
void add_edge(int u, int v, int c) {
    ...
    to[nEdge] = u, cap[nEdge] = c, flow[nEdge] = 0, nxt[nEdge] = fin[v], fin[v] = nEdge++;
    assert(nEdge <= 2*MAXE);
}
